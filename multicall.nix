...
let
  multicall = libjpeg.overrideAttrs (old: {
    ...
    postBuild = (old.postBuild or "") + ''
      ...
      # lib.multicallDispatcherC). Reads multicall/apps.list (written above) and
      # derives each applet's C symbol via `tr -c 'A-Za-z0-9_' '_'`; our mains
      # are renamed to <tool>_main above, which matches for these plain (un-
      # hyphenated) tool names. Invoked at COLUMN 0 so the heredoc terminators
      # in the generated script reach the shell at column 0.
